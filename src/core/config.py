...
class Settings:
    ...
    pubsub_topic: str = ""
    firestore_collection: str = "github_agent_deliveries"
    ...
    def from_env(cls) -> Settings:
        ...
        return cls(
            ...
            allowed_repositories=_get_env_list(
                "ALLOWED_REPOSITORIES", DEFAULT_ALLOWED_REPOSITORIES
            ),
            allowed_installation_ids=tuple(
                int(item)
                for item in _get_env_list("ALLOWED_INSTALLATION_IDS", ())
            ),
            ...
            pubsub_topic=_get_env_str("PUBSUB_TOPIC", ""),
            firestore_collection=_get_env_str(
                "FIRESTORE_COLLECTION", "github_agent_deliveries"
            ),
            max_agent_tool_calls=_get_env_int("MAX_AGENT_TOOL_CALLS", 8),
        )
